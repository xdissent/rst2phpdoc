...
class Writer(writers.Writer):
    ...
    settings_spec = (
        'phpDoc-specific Options',
        None,
        (('Set phpDoc XML document type. '
            'Choices are "refentry", "article", "book", and "chapter". '
            'Default is "refentry".',
            ['--doctype'],
            {'default': 'refentry', 
             'metavar': '<name>',
             'type': 'choice', 
             'choices': ('refentry', 'article', 'book', 'chapter',)
            }
         ),
        )
    )
    ...
    def translate(self):
        visitor = PHPDocTranslator(self.document)
        self.document.walkabout(visitor)
        self.output = visitor.astext()


class PHPDocTranslator(nodes.NodeVisitor):
